<x-layout :$pageTitle :$subtopic>
    @php $editType = $post['id'] == $post['first_post'] ? 'Thread' : 'Post'; @endphp
    <a href="/forum/communityboards">Boards</a> >> <a href="/forum/communityboards/board/{{$post['section']}}">{{config('custom.forum_sections')[$post['section']]}}</a> >> <a href="/forum/communityboards/thread/{{$post['first_post']}}">{{htmlspecialchars($first_post['post_topic'])}}</a> >> <b>Edit {{$editType}}</b><br /><br />
    @if($errors->any())
    ...
    <form action="/forum/communityboards/save" method="POST">
    @csrf
    @method('put')
    <input type="hidden" name="id" value="{{$post->id}}"/>
    <table width="100%"><tr bgcolor="{{config('custom.vdarkborder')}}"><td colspan="2"><font color="white"><b>Edit {{$editType}}</b></font></td></tr><tr bgcolor="{{config('custom.darkborder')}}"><td width="180"><b>Character:</b></td><td>
    <select name="char_id"><option value="0">(Choose character)</option>
    @foreach(auth()->guard('account')->user()->getPlayers as $player)
        <option value="{{$player->id}}"{{old('char_id') == $player->id || $post['author_guid'] == $player->id ? ' selected="selected"' : ''}}>{{$player->name}}</option>
    @endforeach
    </select></td></tr><tr bgcolor="{{config('custom.lightborder')}}"><td><b>Topic:</b></td><td>
    <input type="text" name="topic" value="{{htmlspecialchars(old('topic') ? old('topic') : $post['post_topic'])}}" size="40" maxlength="60"/>{{($editType == "Thread" ? '' : ' (Optional)')}}</td></tr>
    ...
    <textarea rows="10" cols="60" name="text" maxlength="10000">{{old('text') ? old('text') : $post['post_text']}}</textarea><br />(Max. 10,000 letters)</td></tr>
    <tr bgcolor="{{config('custom.lightborder')}}"><td valign="top">Options:</td><td><label>
    <input type="checkbox" name="smile"{{old('smile') || $post['post_smile'] ? ' checked="checked"' : ""}} value="1"/>Disable Smileys in This Post </label></td></tr></table><center>
    <br>
    <input type="submit" value="Edit {{$editType}}" /></center>
    </form>
</x-layout>