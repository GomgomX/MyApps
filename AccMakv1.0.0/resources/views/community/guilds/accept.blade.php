<x-layout :$pageTitle :$subtopic>
    ...
    @if(!isset($action))
        ...
        @else
            ...
            @endif
			<TABLE BORDER=0 CELLSPACING=1 CELLPADDING=4 WIDTH=100%><TR BGCOLOR={{config('custom.vdarkborder')}}><TD CLASS=white><B>Accept invitation</B></TD></TR>
			<TR BGCOLOR={{config('custom.lightborder')}}><TD WIDTH=100%>Select character to join guild:</TD></TR>
			<TR BGCOLOR={{config('custom.darkborder')}}><TD>
			<form style="display:inline-block; margin:0; padding:0;" action="/community/guilds/joinguild" METHOD="post">
            @csrf
            @method('put')
            <input type="hidden" name="id" value="{{$guild_id}}">
			@php sort($list_of_invited_players); @endphp
			@foreach($list_of_invited_players as $invited_player_from_list)
				<input type="radio" name="name" value="{{htmlspecialchars($invited_player_from_list)}}"/>{{htmlspecialchars($invited_player_from_list)}}<br>
            @endforeach
            </TD></TR></TABLE><br>
        ...
        @endif
    @elseif($action == "joined_guild")
        <TABLE BORDER=0 CELLSPACING=1 CELLPADDING=4 WIDTH=100%><TR BGCOLOR={{config('custom.vdarkborder')}}><TD CLASS=white><B>Accept invitation</B></TD></TR>
        <TR BGCOLOR={{config('custom.darkborder')}}><TD WIDTH=100%>Player with name <b>{{htmlspecialchars($player_name)}}</b> has joined guild <b>{{htmlspecialchars($guild_name)}}</b>.</TD></TR></TABLE>
